...
if ( ( ~isa( cons, 'function_handle' ) && ~strcmp( cons, 'combined' ) ) ||  ...
    ...
    if ( ~isa( cons, 'function_handle' ) && ~strcmp( cons, 'combined' ) )
        msg = [ ' DEFT-FUNNEL GLOBALOPT error: the second argument is',   	...
                ' neither a function handle nor the string "combined"!',    ...
                ' Terminating.' ];
    end
    if ( ~isa( objfun, 'function_handle' ) )
        msg = [ ' DEFT-FUNNEL GLOBALOPT error: the first argument is',      ...
                ' not a function handle! Terminating.' ];
    end
    ...
    if ( ~isnumeric( nbcons ) )
        msg = [ ' DEFT-FUNNEL GLOBALOPT error: the number of constraints',	...
