function Lj = deft_funnel_computeLj(sample_set, j, setting)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Desc: Computes the coefficients of the j-th Lagrange polynomial.
%
% Input:
%   - sample_set : the structure of the sample set
...
if (setting.whichmodel == 0)
   ...
   warning off
   Lj = (sample_set.QZ * (sample_set.RZ' \ [zeros(j-1,1);1;zeros(p1-j,1)]))';
...
elseif (setting.whichmodel == 1)
   ...
   if (p1 == n+1 || p1 == q)
      ...
      warning off
      Lj = (sample_set.RZ \ sample_set.QZ' * [zeros(j-1,1);1;zeros(p1-j,1)])';
...
elseif (setting.whichmodel == 2)
   ...
   if (p1 < q)
      ...
      warning off
      Lj = (sample_set.QZ * (pinv(sample_set.RZ') * [zeros(j-1,1);1;zeros(p1-j,1)]))';
...
elseif (setting.whichmodel == 3)
   ...
   % (QZ and RZ are the factors of Z = M)
   warning off
   Lj = (pinv(sample_set.RZ) *  sample_set.QZ' * [zeros(j-1,1);1;zeros(p1-j,1)])';
